...
describe(`Velo External DB Data REST API: ${currentDbImplementationName()}`,  () => {
    ...
    testIfSupportedOperationsIncludes(supportedOperations, [PrimaryKey])('insert api should throw 409 error if item already exists and continue inserting the rest', async() => {
        await schema.givenCollection(ctx.collectionName, [ctx.column], authOwner)
        await data.givenItems([ ctx.items[1] ], ctx.collectionName, authAdmin)
