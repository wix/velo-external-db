...
class SchemaProvider {
    ...
    async list() {
        ...
        const tables = parseTableData( rs.recordsets[0] )
    ...
    async describeCollection(collectionName) {
        ...
        if (rs.recordsets[0].length === 0) {
            throw new CollectionDoesNotExists('Collection does not exists')
        }

        return asWixSchema(rs.recordsets[0].map( this.translateDbTypes.bind(this) ), collectionName)
    }
