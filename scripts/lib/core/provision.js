...
const provisionDb = async (provider, configWriter, { engine, secretId, secretKey, dbName, provisionVariables }) => {
    ...
    await startSpinnerWith(`Writing db config`, async () => await configWriter.writeConfig(secretId, dbCredentials, status.host, dbName, secretKey, provisionVariables, instanceName))
...
const provisionAdapter = async (provider, engine, secretId, configWriter, provisionVariables) => {
    const instanceName = randomWithPrefix('velo-external-db-adapter')

    const { serviceId } = await startSpinnerWith(`Provision Adapter`, async () => await provider.createAdapter(instanceName, engine, secretId, provisionVariables))
...
const main = async ({ vendor, engine, credentials }) => {
    ...
    await provisionDb(dbProvision, configWriter, { engine, secretId, secretKey, dbName, provisionVariables})

    blankLine()
    blankLine()
    info('Provision Adapter')
    await provisionAdapter(adapterProvision, engine, secretId, configWriter, provisionVariables)
